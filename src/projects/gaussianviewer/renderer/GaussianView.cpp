	...
	sibr::ViewBase(render_w, render_h)
{
	...
	std::cout<< "Resolution: " << render_w << "x" << render_h << std::endl;
	...
	// Create space for previous frame data
	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&pre_view_cuda, sizeof(sibr::Matrix4f)));
	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&pre_proj_cuda, sizeof(sibr::Matrix4f)));
	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&pre_cam_pos_cuda, 3 * sizeof(float)));
...
void sibr::GaussianView::onRenderIBR(sibr::IRenderTarget & dst, const sibr::Camera & eye)
{
	...
	else
	{
		...
		sibr::Matrix4f pre_view_mat;
		sibr::Vector3f pre_cam_pos;

		// Compute additional view parameters
		float tan_fovy = tan(eye.fovy() * 0.5f);
		float tan_fovx = tan_fovy * eye.aspect();

		// Copy previous frame-dependent data to GPU
		if(_first)
		{
			CUDA_SAFE_CALL(cudaMemcpy(pre_view_cuda, view_mat.data(), sizeof(sibr::Matrix4f), cudaMemcpyHostToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(pre_proj_cuda, proj_mat.data(), sizeof(sibr::Matrix4f), cudaMemcpyHostToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(pre_cam_pos_cuda, &eye.position(), sizeof(float) * 3, cudaMemcpyHostToDevice));
			cudaMemset(depth_map_cuda, FLT_MAX, mipmap_size * sizeof(float));
			_first = false;
		}
		else
		{	
			// Copy current frame data to previous frame CUDA variables
			CUDA_SAFE_CALL(cudaMemcpy(pre_view_cuda, view_cuda, sizeof(sibr::Matrix4f), cudaMemcpyDeviceToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(pre_proj_cuda, proj_cuda, sizeof(sibr::Matrix4f), cudaMemcpyDeviceToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(pre_cam_pos_cuda, cam_pos_cuda, 3 * sizeof(float), cudaMemcpyDeviceToDevice));

			CUDA_SAFE_CALL(cudaMemcpy(&pre_view_mat, pre_view_cuda, sizeof(sibr::Matrix4f), cudaMemcpyDeviceToHost));
			CUDA_SAFE_CALL(cudaMemcpy(&pre_cam_pos, pre_cam_pos_cuda, 3 * sizeof(float), cudaMemcpyDeviceToHost));

			// Strongly Change:
			float cam_dist = (eye.position() - pre_cam_pos).norm();
			Matrix3f R_view = view_mat.block<3, 3>(0, 0);
			Matrix3f R_pre_view = pre_view_mat.block<3, 3>(0, 0);
			float cos_theta  = ((R_view * R_pre_view.transpose()).trace() - 1) / 2;
			float theta = std::acos(std::min(1.0f, std::max(-1.0f, cos_theta)));

			if (cam_dist > 0.2 || theta > 1)
			{
				cudaMemset(depth_map_cuda, FLT_MAX, mipmap_size * sizeof(float));
			}
		...
		if (!_interop_failed)
		{
			...
			CUDA_SAFE_CALL(cudaGraphicsResourceGetMappedPointer((void**)&image_cuda, &bytes, imageBufferCuda));// OpenGL Alloc
		}
		else
		{
			image_cuda = fallbackBufferCuda;// Cuda Alloc
		}

		float* depthmap = depth_map_cuda;


		int depth_culling_mod = 0;
		if (depthCullingMode == "Max")
			depth_culling_mod = 1;
		else if (depthCullingMode == "Median")
			depth_culling_mod = 2;
		else if (depthCullingMode == "Mean")
			depth_culling_mod = 3;

		float* boxmin = _cropping ? (float*)&_boxmin : nullptr;
		float* boxmax = _cropping ? (float*)&_boxmax : nullptr;

		max_level = sibr::clamp(max_level, 0, 10);

		CudaRasterizer::Rasterizer::forward(
			geomBufferFunc,
			binningBufferFunc,
			imgBufferFunc,
			count, _sh_degree, 16, depth_culling_mod, max_level,
			background_cuda,
...
void sibr::GaussianView::onGUI()
{
	...
	// ImGui::Checkbox("Depth Culling", &_depthCulling);
	if(ImGui::BeginCombo("Depth Culling Mode", depthCullingMode.c_str()))
	{
		if(ImGui::Selectable("None"))
			depthCullingMode = "None";
		if(ImGui::Selectable("Max"))
			depthCullingMode = "Max";
		if(ImGui::Selectable("Median"))
			depthCullingMode = "Median";
		if(ImGui::Selectable("Mean"))
			depthCullingMode = "Mean";
		ImGui::EndCombo();
	}
...
sibr::GaussianView::~GaussianView()
{
	...
	cudaFree(pre_view_cuda);
	cudaFree(pre_proj_cuda);
	cudaFree(pre_cam_pos_cuda);
	cudaFree(background_cuda);
	cudaFree(depth_map_cuda);
